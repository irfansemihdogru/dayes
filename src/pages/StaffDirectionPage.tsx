...
const StaffDirectionPage = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const [audioEnabled, setAudioEnabled] = useState(true);
  const { isDarkMode } = useTheme();
  
  // Get staff and service info from location state
  const { staffName = '', service = '' } = location.state || {};
  
  // Get staff info
  const staffRoomInfo = staffInfo[staffName] || { 
    name: staffName, 
    floor: 1, 
    location: 'karşıda', 
    roomNumber: 1
  ...
  const serviceDisplayName = serviceMap[service] || service;
